...
BEGIN
    IF :NEW.ID IS NULL THEN
        :NEW.ID := TO_CHAR(SYSTIMESTAMP, 'YYMMDD')
...
-- TRIGGER KIỂM TRA EMAIL VÀ PHONE NUMBER CỦA EMPLOYEE
...
-- TRIGGER KIỂM TRA EMAIL VÀ PHONE NUMBER CỦA MEMBER
...
-- TRIGGER TỰ ĐỘNG CHỈNH SỬA ĐIỂM CỦA MEMBER KHI CÓ CẬP NHẬT ĐIỂM
...
-- TRIGGER TỰ ĐỘNG CHỈNH SỬA SỐ LƯỢNG TỒN KHO CỦA INVENTORY KHI THÊM 1 INVENTORY_TRANSACTION
