...
CREATE OR REPLACE PROCEDURE PRC_ADD_ITEM_TO_INVOICE(
    ...
    V_SETS       PLS_INTEGER;
BEGIN
    -- KHÓA HÀNG SẢN PHẨM TRONG INVOICE_DETAIL (NẾU ĐÃ TỒN TẠI)
    BEGIN
        SELECT 1
        INTO V_SETS
        FROM INVOICE_DETAIL
    ...
    -- CẬP NHẬT NET_AMOUNT, KHÓA HÓA ĐƠN ĐỂ CẬP NHẬT TỔNG TIỀN & CHIẾT KHẤU */
    SELECT NET_AMOUNT
    INTO V_TOTAL_SOLD -- TÁI SỬ DỤNG BIẾN
    FROM INVOICE
    WHERE ID = P_INVOICE_ID
        FOR UPDATE; -- KHÓA HÀNG INVOICE
...
AS
    ...
    V_STORE_ID INVOICE.STORE_ID%TYPE;
BEGIN

    -- 1) LẤY NET_AMOUNT, MEMBER_ID, POINT_USED, STORE_ID
    SELECT NET_AMOUNT, MEMBER_ID, POINT_USED, STORE_ID
    INTO V_NET_AMOUNT, V_MEMBER_ID, V_POINT_USED, V_STORE_ID
    FROM INVOICE
    WHERE ID = P_INVOICE_ID;
    ...
    IF V_POINT_USED * 40 > V_NET_AMOUNT THEN
        RAISE_APPLICATION_ERROR(
                -20002,
                'SỐ ĐIỂM SỬ DỤNG (' || V_POINT_USED || ') VƯỢT QUÁ MỨC CHO PHÉP CHO TỔNG TIỀN ' || TO_CHAR(V_NET_AMOUNT)
        );
    END IF;
    ...
    -- AFTER FINISHING THE INVOICE, INSERT INVENTORY TRANSACTIONS FOR ALL PRODUCTS IN INVOICE_DETAIL WITH TYPE 'BÁN HÀNG' (IF UNIT_PRICE > 0) AND 'KHUYẾN MÃI' (IF UNIT_PRICE = 0). USE THE CORRECT STORE_ID FROM THE INVOICE.
    FOR REC IN (SELECT PRODUCT_ID, QUANTITY_SOLD, UNIT_PRICE
                FROM INVOICE_DETAIL
                WHERE INVOICE_ID = P_INVOICE_ID FOR UPDATE) -- KHÓA TỪNG DÒNG CHI TIẾT
        LOOP
            IF REC.UNIT_PRICE > 0 THEN
                INSERT INTO INVENTORY_TRANSACTION (PRODUCT_ID, STORE_ID, TRANSACTION_TYPE, QUANTITY)
                VALUES (REC.PRODUCT_ID, V_STORE_ID, 'BÁN HÀNG', -REC.QUANTITY_SOLD);
            ELSE
                INSERT INTO INVENTORY_TRANSACTION (PRODUCT_ID, STORE_ID, TRANSACTION_TYPE, QUANTITY)
                VALUES (REC.PRODUCT_ID, V_STORE_ID, 'KHUYẾN MÃI', -REC.QUANTITY_SOLD);
            END IF;
        END LOOP;
END PRC_CALC_TOTAL;
/
...
BEGIN
    ...
    IF V_INVOICE.MEMBER_ID IS NOT NULL AND V_INVOICE.STATUS IN ('ĐÃ HOÀN THÀNH', 'ĐÃ HỦY') THEN
        FOR REC IN (
            SELECT POINT_CHANGE
        ...
        FOR ITEM_REC IN (
            ...
            FROM INVOICE_DETAIL
            WHERE INVOICE_ID = P_INVOICE_ID
                FOR UPDATE -- KHÓA CHI TIẾT
            )
            LOOP
                INSERT INTO INVENTORY_TRANSACTION (PRODUCT_ID, STORE_ID, TRANSACTION_TYPE, QUANTITY)
                VALUES (ITEM_REC.PRODUCT_ID, V_INVOICE.STORE_ID, 'ĐIỀU CHỈNH', ITEM_REC.QUANTITY_SOLD);
            END LOOP;
...
CREATE OR REPLACE PROCEDURE PRC_CALC_PAYCHECK(
    ...
    V_COUNT       NUMBER;
BEGIN
    -- KHÓA HÀNG NHÂN VIÊN ĐỂ TRÁNH 2 PHIÊN TÍNH LƯƠNG SONG SONG
    SELECT HOURLY_WAGE
    INTO V_WAGE
    FROM EMPLOYEE
    WHERE ID = P_EMP_ID
    ...
    -- KIỂM TRA CÁC KỲ TRẢ LƯƠNG GIAO CẮT/ĐÈ LÊN KHOẢNG THỜI GIAN THAM SỐ
    SELECT COUNT(*)
    INTO V_COUNT
    FROM PAYCHECK
...
BEGIN
    ...
    -- KHÓA BẢN GHI CỦA P_MANAGER_ID ĐỂ TRÁNH THAY ĐỔI SONG SONG
    SELECT MANAGER_ID
    INTO V_MGR_ID
    FROM EMPLOYEE
